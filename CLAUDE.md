...
- **Meds (`meds/`) is one-tap intake logging, and the only feature that reuses columns instead of adding them.** The brief was written against the old Notion schema ("dose belongs in the title and Notes, not a new field"), so it was honoured literally: no migration, no new columns. One `supplement` EventType (category `intake`) carries every substance, and three existing columns are aliased for it in `lib/db/rows.ts` + `lib/store/events.ts` — `kind` → `substance` (registry id), `description` → `dose` (text, e.g. `"25 mg"`), `notes` → `note`. `patchMismatch` blocks those three on any other type, because letting one through would corrupt a caffeine or meal row. Dose text is parsed/formatted only in `lib/substances/format.ts` (tested) — don't hand-roll it elsewhere.
- **The substance registry is a JSON file, not a table.** `config/substances.json` is the bundled seed; `data/substances.json` on the Docker volume (or `$SUBSTANCES_PATH`) overrides it, so adding a substance in prod is one file edit plus a restart — no rebuild, no code change. Served by `GET /api/substances`, cached for the process. `POST /api/event` checks membership against it; `lib/validation.ts` only checks the slug shape, so it stays pure and synchronously testable.
- The row label `intake:{type} — {dose}{unit}` is the old Notion title convention. There is no title column — it's composed at render time in both presentation modules and in the undo toast.
- Meds ghosts follow the nutrition rule (`components/meds/ghost.ts`): clear per-row when the real event lands, **never** blanket-clear on refresh. A dose queued offline has no server row, and blanket-clearing hides it from the one list that exists to prevent double-dosing.
...
- **Two tables preserve history the app doesn't render.** `events.notes` holds free text that existed on 139 wake/sleep/caffeine/mood/energy rows whose old reader ignored the field. `legacy_events` holds the 170 pre-app rows whose `Type` predates the current nine (`journal`, `snooker`, `work`, `travel`, `melatonin`, `legacy_note`…), each with its untouched Notion property blob in `properties jsonb`. `legacy_events` is read by no route — it exists so the migration lost nothing. Don't "clean them up". **`events.notes` is no longer read-only:** `supplement` rows write and read it (see below), and `fromEventRow` only surfaces it for that type, so the 139 legacy notes stay invisible.
