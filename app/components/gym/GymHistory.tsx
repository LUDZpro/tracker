...
import { gymSummary, GYM_COLOR, GYM_ICON } from './gymPresentation';
  ...
  return (
    <li className={styles.rowSlot}>
      ...
      >
        ...
        <span className={styles.rowIcon} style={{ color: GYM_COLOR }} aria-hidden>
          {GYM_ICON}
        </span>
        <span className={styles.rowLabel}>{gymSummary(ev)}</span>
...
/** Reverse-chronological gym session history, day-separated, cursor-paged. */
export default function GymHistory({
  ...
  return (
    <>
      <ul className={styles.todayList} aria-label="Logged gym sessions, newest first">
        {items}
      </ul>
      {hasMore && (
